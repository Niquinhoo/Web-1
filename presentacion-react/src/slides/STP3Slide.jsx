      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-full">
        <div>
          ...
          <p className="text-lg text-textMuted mb-8 leading-relaxed">
            De <strong className="text-white">arrays en memoria</strong> a
            <strong className="text-white"> better-sqlite3</strong> real.
            Schema SQL, transacciones ACID, bootstrap idempotente y
            <strong className="text-white"> 7 User Stories</strong>.
          </p>

          <div className="grid grid-cols-2 gap-3">
            <FeatureCard icon={<Database className="text-blue-400" />} title="5 Tablas SQL" desc="categories, products, users, orders, order_items con FOREIGN KEYs." />
            <FeatureCard icon={<RefreshCw className="text-purple-400" />} title="Bootstrap ACID" desc="Migración atómica de users_legacy + seed idempotente con transacciones." />
            <FeatureCard icon={<Server className="text-green-400" />} title="Carrito Híbrido" desc="Sesión guarda IDs, DB provee precios reales. Nunca confía en memoria." />
            <FeatureCard icon={<ShieldCheck className="text-red-400" />} title="normalizeId() + DB" desc="Valida formato (400) Y existencia en DB (404) en una sola función." />
            <FeatureCard icon={<Search className="text-cyan-400" />} title="SQL Real" desc="LIKE, ORDER BY RANDOM(), LOWER() — no más Array.filter()." />
            <FeatureCard icon={<Database className="text-amber-400" />} title="prepare()" desc="Statements precompilados con bound parameters (?). Seguro y eficiente." />
          </div>
        </div>
        ...
        <div className="glass-panel p-8 h-[520px] flex flex-col justify-between">
          ...
          <div>
            <div className="p-3 bg-emerald-900/20 border border-emerald-500/20 rounded-lg text-sm text-emerald-200 mb-4">
...
function TheoryLine({ concept, code }) {
  return (
    <div className="flex items-start gap-3 text-textMuted">
      ...
      <span className="text-indigo-300 font-medium shrink-0 w-52">{concept}</span>
