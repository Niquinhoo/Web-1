    ...
    <Slide className="justify-center">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-full">
        <div className="order-2 lg:order-1 glass-panel p-8 h-[520px] flex flex-col justify-between">
          ...
          <div>
            <div className="p-3 bg-purple-900/20 border border-purple-500/20 rounded-lg text-sm text-purple-200 mb-4">
        ...
        <div className="order-1 lg:order-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FeatureCard icon={<Share2 className="text-blue-400" />} title="Controllers" desc="Capa delgada HTTP que delega al service sin tocar datos." />
            <FeatureCard icon={<Blocks className="text-purple-400" />} title="10 Operaciones" desc="productsService con búsqueda, categorías, sort y normalización." />
            <FeatureCard icon={<Archive className="text-green-400" />} title="Sesión Real" desc="express-session con { productId, quantity } — precios al renderizar." />
            <FeatureCard icon={<AlertTriangle className="text-red-400" />} title="400/404/500" desc="Tres códigos HTTP con vistas EJS dedicadas." />
            <FeatureCard icon={<Shield className="text-amber-400" />} title="Validación 422" desc="Register con validación dual frontend + backend." />
            <FeatureCard icon={<Search className="text-cyan-400" />} title="19 US" desc="Todas cumplidas: catálogo, carrito, auth, errores, layout." />
          </div>
        </div>
      </div>
    </Slide>
